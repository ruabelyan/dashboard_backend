 ...
 */
router.get('/', (req, res) => {
    db.all('SELECT * FROM categories ORDER BY name ASC', [], (err, rows: any[]) => {
        if (err) {
...
router.post('/', (req, res) => {
    ...
    db.run(
        'INSERT INTO categories (id, name, description, image, is_default) VALUES (?, ?, ?, ?, ?)',
        [id, name, description, image || null, 0],
        function (err) {
            if (err) {
                console.error('Database error:', err);
...
router.put('/:id', (req, res) => {
    ...
    db.get('SELECT * FROM categories WHERE id = ?', [id], (err, category: any) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ error: 'Database error' });
        }
        ...
        db.run(
            'UPDATE categories SET name = ?, description = ?, image = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
            [
                name || category.name,
...
router.get('/:id/geckos', (req, res) => {
    ...
    db.all(
        'SELECT * FROM geckos WHERE category = ? ORDER BY id ASC',
        [id],
        (err, geckos) => {
            if (err) {
                console.error('Database error:', err);
