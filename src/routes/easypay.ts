...
router.get('/clients', authenticateToken, (req, res) => {
  ...
  // Get total count
  db.get('SELECT COUNT(*) as total FROM easypay_clients', (err, countResult: any) => {
    if (err) {
    ...
    db.all(
      'SELECT * FROM easypay_clients ORDER BY created_at DESC LIMIT ? OFFSET ?',
      [limit, offset],
      (err, clients: any[]) => {
        if (err) {
          console.error('Database error:', err);
...
router.get('/clients/search', authenticateToken, (req, res) => {
  ...
  db.get(
    `SELECT COUNT(*) as total FROM easypay_clients 
...
router.get('/clients/:id', authenticateToken, (req, res) => {
  ...
  db.get(
    'SELECT * FROM easypay_clients WHERE id = ?',
    [clientId],
    (err, client: any) => {
      if (err) {
        console.error('Database error:', err);
...
router.post('/clients', authenticateToken, validateRequest(easypayClientSchema), (req, res) => {
  ...
  db.get('SELECT id FROM easypay_clients WHERE client_id = ?', [clientData.client_id], (err, existingClient: any) => {
    if (err) {
      console.error('Database error:', err);
      return res.status(500).json({ error: 'Database error' });
    }
...
router.put('/clients/:id', authenticateToken, validateRequest(easypayClientSchema), (req, res) => {
  ...
  db.get('SELECT id FROM easypay_clients WHERE id = ?', [clientId], (err, existingClient: any) => {
    if (err) {
      console.error('Database error:', err);
      return res.status(500).json({ error: 'Database error' });
    }
...
router.post('/import-excel', authenticateToken, upload.single('file'), async (req: AuthRequest, res) => {
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (ssnIndex !== -1 && row[ssnIndex]) client.ssn = row[ssnIndex].toString().trim();
      if (isResidentIndex !== -1 && row[isResidentIndex]) client.is_resident = row[isResidentIndex].toString().trim();
      if (passwordIndex !== -1 && row[passwordIndex]) client.password = row[passwordIndex].toString().trim();
      if (pinIndex !== -1 && row[pinIndex]) client.pin = row[pinIndex].toString().trim();
      if (addressIndex !== -1 && row[addressIndex]) client.address = row[addressIndex].toString().trim();
      if (dateOfBirthIndex !== -1 && row[dateOfBirthIndex]) client.date_of_birth = row[dateOfBirthIndex].toString().trim();
      if (isEnabledIndex !== -1 && row[isEnabledIndex]) client.is_enabled = row[isEnabledIndex].toString().trim();
      if (phone2Index !== -1 && row[phone2Index]) client.phone2 = row[phone2Index].toString().trim();
      if (genderIndex !== -1 && row[genderIndex]) client.gender = row[genderIndex].toString().trim();
      if (languageIndex !== -1 && row[languageIndex]) client.language = row[languageIndex].toString().trim();
      if (phoneVerifiedIndex !== -1 && row[phoneVerifiedIndex]) client.phone_verified = row[phoneVerifiedIndex].toString().trim();
      if (emailVerifiedIndex !== -1 && row[emailVerifiedIndex]) client.email_verified = row[emailVerifiedIndex].toString().trim();
      if (openDateIndex !== -1 && row[openDateIndex]) client.open_date = row[openDateIndex].toString().trim();
      if (isVerifiedIndex !== -1 && row[isVerifiedIndex]) client.is_verified = row[isVerifiedIndex].toString().trim();
      if (orderedRegistrationIndex !== -1 && row[orderedRegistrationIndex]) client.ordered_registration = row[orderedRegistrationIndex].toString().trim();
      if (verificationDateIndex !== -1 && row[verificationDateIndex]) client.verification_date = row[verificationDateIndex].toString().trim();
      if (registrationTypeIndex !== -1 && row[registrationTypeIndex]) client.registration_type = row[registrationTypeIndex].toString().trim();
