...
export class EasyPayDataImporter {
   ...
   */
  async importAll(options: ImportOptions = {}): Promise<ImportStats> {
   ...
   */
  private async analyzeData(clients: any[]): Promise<ImportStats> {
   ...
   */
  private createBatches(clients: any[], batchSize: number): any[][] {
    const batches = [];
   ...
   */
  private async processBatch(batch: any[], options: ImportOptions): Promise<void> {
   ...
   */
  private async processClient(client: any, options: ImportOptions): Promise<void> {
    try {
   ...
   * Check if client already exists
   */
  private async checkExistingClient(clientId: string): Promise<any> {
    return new Promise((resolve, reject) => {
      db.get(
        'SELECT id FROM easypay_clients WHERE client_id = ?',
        [clientId],
        (err, row: any) => {
          if (err) reject(err);
          else resolve(row);
   ...
   * Insert new client
   */
  private async insertClient(client: any): Promise<void> {
    return new Promise((resolve, reject) => {
      db.run(
   ...
   */
  private async updateClient(client: any): Promise<void> {
    return new Promise((resolve, reject) => {
      db.run(
